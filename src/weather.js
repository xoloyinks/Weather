...
export default function Weather() {
    ...
    useEffect(() => {
        fetch(`https://api.weatherapi.com/v1/current.json?key=c379f3114aa5430bbc7165817231303&q=${city} ${country}&aqi=no`)
        .then(res => res.json())
        .then((data) => {
        ...
        <section className='relative z-50 w-screen h-screen'>
            ...
            <div className='absolute top-0 bottom-0 left-0 right-0 backdrop-blur-lg text-xl text-white bg-white/3'>
                ...
                <div className='px-3 py-4 flex flex-col gap-3'>
                    <p><input type="text" onChange={(e) => setDevcity(e.target.value)} value={devCity} placeholder='Enter your city' className='input p-3 text-sm text-black font-bold w-full rounded' /></p>
                    <p><input type="text" onChange={(e) => setDevcountry(e.target.value)} value={devCountry} placeholder='Enter your country' className='input p-3 text-sm text-black font-bold w-full rounded' /></p>
                    <button onClick={() => {setCity(devCity); setCountry(devCountry)}} className='px-8 py-4 w-[fit-content] mx-auto bg-gray-900 rounded-full text-sm'>Check</button>
                </div>
